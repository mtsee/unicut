...
class MP4Demuxer {
  ...
  constructor(uri, { onConfig, onChunk, setStatus }) {
    ...
    this.#file.onError = error => {
      console.error('MP4Box demux error:', error);
  ...
  #onReady(info) {
    ...
    const hasAudio = info.tracks.find(d => d.type === 'audio');

    if (!track) {
      console.warn('编码格式不支持前端解码');
      this.#onConfig({
        error: true,
        hasAudio,
      });
  ...
  #onSamples(track_id, ref, samples) {
    // console.log('onSamples', track_id, ref, samples);
